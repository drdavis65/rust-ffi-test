; *** IR Dump After SLPVectorizerPass on axpy_pts_restricted ***
...
for.body:                                         ; preds = %middle.block, %for.body.preheader, %for.body
  ...
  %5 = load <2 x float>, ptr %arrayidx, align 4, !tbaa !12
  %6 = load <2 x float>, ptr %arrayidx1, align 4, !tbaa !12
  %7 = insertelement <2 x float> poison, float %alpha, i32 0
  %8 = shufflevector <2 x float> %7, <2 x float> poison, <2 x i32> zeroinitializer
  %9 = call <2 x float> @llvm.fmuladd.v2f32(<2 x float> %8, <2 x float> %5, <2 x float> %6)
  store <2 x float> %9, ptr %arrayidx1, align 4, !tbaa !12
